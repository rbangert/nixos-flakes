...
class ChatTab {
    ...
    static async activate_from_outside(question, editor, attach_default, use_model, use_model_function) {
        ...
        panel.webview.onDidReceiveMessage(async (data) => {
            switch (data.type) {
                ...
                case "question-posted-within-tab": {
                    await free_floating_tab.chat_post_question(data.chat_question, data.chat_model, data.chat_model_function, data.chat_attach_file);
                    break;
                }
                case "stop-clicked": {
    ...
    async chat_post_question(question, model, model_function, attach_file) {
        if (!this.web_panel) {
            return false;
        }
        ...
        request.supply_stream(...fetchAPI.fetch_chat_promise(cancelToken, "chat-tab", this.messages, model_function, model, [], third_party));
    }
...
async function chat_model_set(chat_model, model_function) {
    let context = global.global_context;
    if (!context) {
        return;
    }
